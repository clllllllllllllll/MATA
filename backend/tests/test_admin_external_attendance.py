...
class FakeAdminExternalAttendanceSession:
    def __init__(self) -> None:
        ...
        self.catalogue_programmes_by_posting = {
            "TTSHCardio": {"DR"},
            "TTSHGerMed": {"GERI"},
        }
    ...
    def _filtered_rows(self, payload: dict) -> list[dict]:
        ...
        if "programme_scope" in payload:
            ...
            rows = [
                row
                for row in rows
                if self.catalogue_programmes_by_posting.get(row["posting_code"], set()) & scope
            ]
        if payload.get("programme_code"):
            rows = [
                row
                for row in rows
                if payload["programme_code"] in self.catalogue_programmes_by_posting.get(row["posting_code"], set())
            ]
