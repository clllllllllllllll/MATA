...
| **Clawback engine** (`clawback.py`) | 📋 Planned | BL-10 specified; generated by future final close/freeze |
...
Phase 5B uses a confirmed two-stage rollout. Stage 1 implements the generic mapping service/API/frontend and seeds exactly 28 TTSH rows as pending with null posting codes and zero active mappings; there is no GERI exception. TTSH and all programmes remain visible, but no registration can be submitted. Stage 2 waits for one complete owner-approved list and activates all 28 rows together through one validated transactional data-only Alembic migration. Future KTPH, WH, and other institutions require configuration rows only.
...
5. Clawback — read-only, generated by future final close/freeze, visible to admin/PC only
...
- Hibernation triggered at two points: (1) RDB upload, (2) future final close/freeze. Not lazily on compliance read.
...
| `GET /admin/reports/clawback` | Clawback 5th tab | Read-only; generated by future final close/freeze |
