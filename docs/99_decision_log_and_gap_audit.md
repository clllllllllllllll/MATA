...
- **Consequences for codebase:** `surplus_ledger` rows are period-scoped via `reporting_period_id`. Period close sets `is_hibernating = true` on all rows. New period starts with zero surplus.
...
#### Decision: Clawback tab — 5th tab, generated at period close
- **Status:** ✅ Confirmed
- **Decision:** Clawback is the 5th tab in admin/PC dashboard. Read-only. Generated at period close via `clawback_records` table (BL-10). All rows shown including suppressed (Extension, R7) with amount = 0.
...
- **Consequences for codebase:** `PUT /admin/reporting-periods/{id}/close` generates `clawback_records`. `GET /admin/reports/clawback` reads from that table. Suppressed rows included with `clawback_suppressed_reason` displayed.
...
- **Reasoning:** Freezing compliance state at period close ensures historical reports are stable and not affected by subsequent data changes.
- **Alternatives considered:** Always re-query live tables — rejected, compliance would change retroactively.
- **Consequences for codebase:** `PUT /admin/reporting-periods/{id}/close` generates snapshots. `GET /admin/exports/period-snapshot/{snapshot_id}` serves historical data. Reopen replaces existing snapshot on next close.
- **Reference file and section:** `schema.md` § `period_snapshots`; `api.md` § PUT `/admin/reporting-periods/{id}/close`
- **Do not change without PM/stakeholder approval:** Yes
...
- **Can development proceed?** Yes — decision needed before first period close, not before development.
...
| 19 | Clawback tab: 5th tab, generated at period close | Reporting requirement | PM | `clawback.py`, admin dashboard |
