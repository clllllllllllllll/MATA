...
test('Secretary schedule uses exact active source IDs from the selected programme', () => {
  ...
  assert.match(scheduleSource, /durationIsMapped/)
  assert.match(scheduleSource, /temporary one-hour duration/)
  assert.match(scheduleSource, /automatically update its duration and end time/)
  assert.match(scheduleSource, /TTF mapping/)
...
test('Secretary Teaching Name page keeps mobile and dialog controls accessible', () => {
  ...
  assert.match(pageSource, /placeholder="Search Name of Teaching"/)
  assert.match(pageSource, /aria-label="Search Name of Teaching"/)
  assert.match(pageSource, /className="sr-only">Search Name of Teaching<\/span>/)
