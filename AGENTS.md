...
- **70% threshold is at the POSTING level**, aggregated across all session types for that posting. The unrounded posting percentage is canonical; monthly percentages are display-only.
- **Persistent surplus is derived audit state per (resident, physical posting, session_type, reporting_period).** Recompute it idempotently as `max(cumulative raw eligible attendance - cumulative target_100, 0)` and replace the ledger value. It is pre-tag-reallocation, hibernates when no active phase remains, resumes/recomputes on return, resets at a period boundary, and is never added back to attendance.
- **Tag-based reallocation uses raw session counts before final capping.** Within one physical posting, R-year context, and tag prefix, earlier alphabetical tags donate to later tags one-for-one. Donor supply is raw achieved above the session type's 70% target; recipient demand is only the sessions needed to reach its 70% target. Decrement supply after every transfer, never transfer hours or cross postings/R-year contexts, and cap each session type at `target_100` after all transfers.
...
- **FormF1 is the final authoritative active/inactive source for compliance.** Records remain stored by calendar month, but compliance uses the FormF1 month corresponding to the resolved AY bucket label for both numerator and denominator. Do not split/prorate an AY bucket or use the event's raw calendar month when it differs from the bucket label.
...
- **Active/inactive from FormF1 gates the compliance denominator.** An AY bucket is excluded from both numerator and denominator when the FormF1 row selected by that bucket label has `is_active = false`. `Active` and `Extension` are active; `Inactive`, blank, `NULL`, and whitespace-only monthly cells are inactive and valid resident rows persist those blank-month records. Employed residents are Active in FormF1.
- **Unknown non-blank FormF1 monthly statuses are warning-only.** They preserve `status_raw`, retain the active fallback, and create a persisted warning with the unknown value and Excel cell reference. Blank monthly cells are recognised inactive values and do not create an unknown-status warning.
...
- **The AY bucket label selects FormF1.** For example, every date in the `Jul-26` AY bucket uses July FormF1, including an event on 3 August if that bucket ends on 3 August. The next AY bucket uses its own label's FormF1 month.
...
- **Multi-posting rule outcomes are distinct.** `main_posting` collapses source codes to the configured existing `main_posting_code`; `combine` persists one row under the configured existing canonical combined posting code and uses that code's TTF rows; `half_month` persists both original postings with `active_months_weight = 0.5` and leaves each TTF `monthly_target` unchanged. Apply the half-month factor exactly once.
...
- **R year required flag drives TTF matching.** Programmes where `r_year_required = false` use `r_year = 'ALL'` as a sentinel value in both `resident_postings` and `teaching_targets`. The TTF matcher checks `r_year == 'ALL' OR r_year == resident_r_year`. SPORTSMED and PALLMED are exceptions to the old no-R-year list: both require R year, store/use R4-R6 directly, have `is_subspecialty = false`, and never remap to SS1-SS3. See `docs/business-logic.md` § BL-11.
- **Mid-period R-year contexts are capped separately.** Resolve the phase covering each event, calculate and cap each `(physical posting, session_type, r_year)` context with only its correctly weighted active months, then sum the separately capped achievements and targets into the final posting result. Never merge raw attendance before those caps or duplicate posting-wide months across R-year rows.
- **Percentage is the canonical compliance predicate.** `met_70pct = percentage >= 0.70` using the unrounded posting percentage. `target_70 = ceil(target_100 * 0.70)` remains a displayed whole-session target; it does not override the percentage predicate for fractional targets. Green is at least 70%, amber is 50% to below 70%, and red is below 50%.
- **Clawback is explicitly deferred.** A future candidacy check must reuse the unrounded posting percentage, but norm rates/effective dating, funding R-year, financial classification, suppression precedence, grouped identity, billing attribution, missing-rate behavior, rounding/precision, and final-close transaction/rerun/idempotency are not specified and must not be invented.
- **Distinct-event overlaps are rejected at submission time.** For the same resident, reject a later submitted event if its interval overlaps an already accepted distinct event, while preserving the earlier attendance unchanged. This is separate from `UNIQUE(resident_id, event_id)` for duplicate submission of the same event.
...
- **ORTHO weekend session mutation is exact-type, adjusted-time, Saturday-only, and read-time.** Preserve raw rows. Only `NHG Orthopaedic Surgery Residency Teaching [3h]` is shortened by two hours from its original end time and projected as `National Didactics & Department Teaching [1h]`; then test the adjusted interval against the Saturday 08:30–10:30 acceptance window. Sunday remains excluded and other ORTHO types are never mutated.
...
- **Canonical catalogue names are scoped, not fuzzy-matched.** Scheduled event creation uses standardized dropdown values such as `Journal Club`. The same canonical name may exist at several postings and map differently; resolve by reporting period, resident programme, assigned/compliance posting, posting R-year, and canonical name. Case/spacing duplicate cleanup inside one TTF scope is upload/event-option data quality, not a Phase 6 compliance blocker. Do not introduce fuzzy matching.
- **Native-programme events outside the assigned posting project into the assigned posting.** Count an approved event once as `Department/Programme Teaching [1h]`, using the assigned posting's TTF target. Do not attribute it to the creator's posting or `programmes.native_teaching_posting_code`, and do not create a separate native-programme result. Preserve the event; this is read-time projection.
...
- **External posting schedules carry programme provenance per row.** Each `external_resident_postings` row stores the validated `programme_code` and backend-resolved `posting_code` for its date range. New registration, schedule-replacement, and compatibility writes must preserve both values. The column remains nullable only for unresolved legacy rows; backfill only a uniquely resolvable programme and never choose the first mapping candidate. A null legacy programme grants no Programme PC-event visibility.
- **Non-NHG scheduled-event visibility uses the exact date-matched external schedule row.** A Non-NHG Resident may see and submit a normal Secretary-created event only when its `posting_code` matches the schedule row and `created_for_programme_code IS NULL`, subject to `supports_secretary_events` and the existing date/reporting-period/status/submission rules. A Programme PC-created event requires exact matches on both `event.posting_code = schedule.posting_code` and `event.created_for_programme_code = schedule.programme_code`. Never infer programme identity from posting prefixes, institution metadata, targets, catalogue rows, native teaching mappings, fuzzy matching, or a first candidate: AIM/IM may share `TTSHGenMed`, and GS/SIG may share `TTSHGenSrg`.
...
| Combined posting event ownership | For a `combine` rule, component-site events resolve to the configured existing canonical combined posting code. One `resident_postings` row and the combined posting's TTF rows drive one compliance result; no separate component results are produced. Event rows retain their original creator posting. |
...
| Active/inactive source | FormF1 is final. Records remain calendar-month keyed, but the resolved AY bucket label selects the FormF1 month for both numerator and denominator; no split or event-calendar-month fallback. |
| Ad-hoc teaching | Residents submit through the dedicated endpoint with `is_adhoc = true`. Countable NHG ad-hoc sessions project to `Department/Programme Teaching [1h]` under the assigned posting; Non-NHG rows remain isolated. |
...
| R year not needed programmes (BL-11) | Resolved. 20 programmes have `r_year_required = false` and use `r_year = 'ALL'`; 8 require R year. SPORTSMED and PALLMED require and retain R4-R6, have `is_subspecialty = false`, and do not use SS remapping. 4 programmes have `rdb_alias` for RDB name normalisation. See `docs/business-logic.md` § BL-11. |
| Mid-period R-year transition | Resolve and cap each correctly weighted physical-posting/session/R-year context separately, then sum capped achievements and targets into the final posting result. Do not duplicate months or merge raw counts before capping. |
| ORTHO weekend mutation | Exact original 3h type only. At read time subtract two hours from the raw end time, project to the 1h type, then test the adjusted Saturday interval against 08:30–10:30. Sunday is excluded; other ORTHO types are not mutated and require any separately applicable acceptance rule. |
...
| Clawback tab | Future admin/PC placeholder only. Calculation, persistence, response, suppression, and final-close generation behavior remain deferred. |
...
| Tag-based reallocation | Use raw achieved session counts before final capping; sort tags alphabetically (A1→A2→A3); flow one-for-one from earlier to later tags within one physical posting/R-year context/prefix; decrement donor supply; then cap every type at its own `target_100`. Hours never flow. |
| Persistent surplus | Derived pre-reallocation audit state: `max(cumulative raw eligible attendance - cumulative target_100, 0)` per resident/physical posting/session type/period. Replace idempotently, never add it back to attendance, and recompute on return. |
| Fractional compliance | Unrounded `percentage >= 0.70` is the canonical predicate. `target_70 = ceil(target_100 * 0.70)` is display-oriented; a capped 100% fractional result cannot fail the ceiling comparison. |
| Distinct-event overlap | Reject the later submitted event when it overlaps an already accepted distinct event for the resident; preserve the earlier attendance. Same-event uniqueness remains separate. |
| Native-programme event attribution | Approved events outside the assigned posting count once as `Department/Programme Teaching [1h]` under the assigned posting's TTF target through read-time projection. |
| Catalogue matching | Canonical dropdown name plus reporting period, programme, assigned/compliance posting, posting R-year, and name determine the mapping. Cross-posting same names are valid; case/spacing cleanup is data quality, not a compliance blocker; no fuzzy matching. |
